...
def _load_resume_utils():
    """Import resume utilities lazily so the web app can still boot without AI extras."""
    try:
        ...
        logging.exception("Resume utilities are unavailable")
        ...
        if 'resume' in request.FILES:
            ...
            if rag_service.initialize_rag(file_path):
                ...
                try:
                    ...
                    if import_error:
                        raise RuntimeError(
                            "Basic resume parsing is unavailable. "
                            "Install the local dependencies from requirements-local.txt."
                        )
